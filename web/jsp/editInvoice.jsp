...
$(document).ready(function(){
    ...
    //edit special
    $('#allbox').attr("checked","checked").change();
    $('.dispatching').change();
    $('.taxCheckBox').change();
...
<tr>
    <td colspan="4"><table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <c:forEach items="${invoiceBean.invoice.invoiceDetail}" var="invoicedetail" varStatus="loop" >
            <c:forEach items="${invoiceBean.invoice.order.orderDetail}" var="orderdetailarray" varStatus="loop1"  >
                <c:if test="${invoicedetail.product.id eq orderdetailarray.product.id}">
...
<tr>
    <td colspan="4"><table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <c:forEach items="${invoiceBean.invoice.taxes}" var="tax" varStatus="loop">
            <c:choose>
                <c:when test="${tax.applicableOn ne null}">
                    <tr><td width="40%" align="left" valign="top">&nbsp;</td>
                        <td width="3%" align="center" valign="top" class="foreach_table_firstth"><div style="margin-top:5px;">
                            <s:checkbox name="taxchkbx" id="taxCheckBox${loop.index}" class="taxCheckBox" checked="true"/>
                        </div></td>
                        <td width="11%" valign="top"  class="foreach_table_th"><div style="color: #ff0000; font-family: Verdana; font-size:10px; margin-top:5px; margin-right:3px; font-size: 12px;" class="labels">
                            <s:text name="invoice.taxes[${loop.index}].taxName" value="${tax.taxName}" id="taxName${loop.index}" size="15" readonly="readonly" style="margin-top:0px ; border:0px; text-align:right;   font-size: 12px;"/></div></td>
                        <td width="5%" valign="top" class="foreach_table_th"><div style="color: #ff0000; font-family: Verdana; font-size:10px; margin-top:5px; margin-right:3px; font-size: 12px;" class="labels">
                            <s:text name="invoice.taxes[${loop.index}].previousTaxPercent" value="${tax.previousTaxPercent}" id="taxPercentage${loop.index}" readonly="readonly" size="15" maxlength="10" style="margin-top:0px ; border:0px; text-align:right;   font-size: 12px;"/></div></td>
                        ...
                        <td width="13%" valign="top" class="foreach_table_th"><div style="color: #ff0000; font-family: Verdana; font-size:10px; margin-top:5px; margin-right:3px; font-size: 12px;" class="labels">
                            ...
                            <s:select id="applicableOn${loop.index}" name="invoice.taxes[${loop.index}].applicableOn" class="dropdown applicableOn">
                                <option  value="0">---Select Applicable On---</option>
                                <c:choose>
                                    <c:when test="${tax.applicableOn eq 'totalAmount'}">
                                        <option value="totalAmount" selected="selected">Total Amount</option>
                                    </c:when>
                                    <c:otherwise>
                                ...
                                <c:forEach items="${invoiceBean.taxlst}" var="orderloop" >
                                    <c:choose>
                                        <c:when test="${orderloop.name ne tax.taxName}">
                                            <c:choose>
                                                <c:when test="${orderloop.name eq tax.applicableOn}">
                                                    <option value="<c:out value="${orderloop.name}"/>" selected="selected"><c:out value="${orderloop.name}"/></option>
                                                </c:when>
                                                <c:otherwise>
