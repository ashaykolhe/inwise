            ...
            function selectItem()
            {
                ...
                if(!$("#restoreallceckboxid").is(':checked'))
                {
                  ...
                  for(var i=1;i<=temp;i++)
                   ...
                   if(flagchk==0)
                   {
                         alert("Please select the User to Restore.");
                         return false;
                   }
                }
...
<s:useActionBean beanclass="com.inwise.action.RestoreActionBean" var="list" event="restoreUserLink"></s:useActionBean>
    <% request.setAttribute("userlst",list.getUserlst());
...
<s:layout-render name="/layout/_base.jsp">
   <s:layout-component name="left-menu">
    ...
    <tr><td align="left" class="pageheading" valign="top">
      <div class="sub_heading" >Restore User</div>
   ...
   <table class="second_table"  ><tr><td colspan="2">
               ...
               <c:if test="${actionBean.restorelistempty eq false}">
                <d:table name="userlst" id="user1" pagesize="10" class="disp" requestURI="/User.action">
                    ...
                    <d:column title="Select User">
                      <s:checkbox name="userListForRestore[${user1_rowNum}]" value="${user1.user_id}" id="singleitemcheckboxid${user1_rowNum}" ></s:checkbox>
                    </d:column>
    ...
    <d:column property="username" title="User Name"  />
       ...
       <d:column property="role.name" title="Role"  />
                   ...
                   <div id="count" style="display:none;">${user1_rowNum}</div>

                    <div style="margin-left:6px">
              <s:checkbox name="restoreAll" id="restoreallceckboxid" class="labels" value="restoreall"></s:checkbox>Restore All Users<span style="color:#FF0000"></span>

                    <s:submit name="restoreUser"  value="Restore All" id="restoreallbutton" class="buttonbig" style="margin-left:6px"></s:submit>
                   <s:submit name="restoreUser"  value="Restore Selected" id="restorebuttonid" class="buttonbig1" style="margin-left:6px" ></s:submit>
                  </div>
               </c:if>
               <c:if test="${actionBean.restorelistempty eq true}">
                   No Users Found For Restore.
                   </c:if>
