...
<s:useActionBean beanclass="com.inwise.action.UserActionBean" var="userBean" event="updateUserLink"></s:useActionBean>
<% request.setAttribute("userlst",userBean.getUserlst());
%>
   ...
   <div align="right" style="margin-left: 1px;" class="labels">Please Select User Name<span style="color:#FF0000"> *</span></div></td>
		 ...
		 <s:select name="id" id="userdropdown" class="dropdown">
            ...
            <c:forEach items="${userlst}" var="userloop" varStatus="loop" >
                   <c:choose>
                  <c:when test="${actionBean.user.id eq userloop.id}">
			            <option value ="<c:out value="${actionBean.user.id}"/>" selected="selected"> <c:out value="${actionBean.user.username}"/></option>
                  </c:when>

                  <c:otherwise>
              <option value ="<c:out value="${userloop.id}"/>"> <c:out value="${userloop.username}"/></option>
                </c:otherwise>
                  </c:choose>
    ...
    <tr>
      ...
      <s:hidden name="user.deleted" value="0"/>
          <s:hidden name="user.id" value="${userBean.id}"/>
      </div> </td>
        <td width="22%" align="left" valign="top" ><s:hidden id="updateusername" name="user.id" class="textbox"></s:hidden><s:errors field="item.name"/> </td>
        ...
        <td align="left" valign="top" >
			 ...
			 <s:select name="user.role.id" id="roledropdown" class="dropdown">
             	...
             	<option value ="<c:out value="${role.id}"/>"> <c:out value="${role.name}"/></option>
                     ...
                     <td align="left" valign="top" >
                     &nbsp;
                         <s:useActionBean id="beanId" beanclass="com.inwise.action.RoleActionBean"></s:useActionBean>
    ...
    <s:reset name="reset" value="Reset"></s:reset>   &nbsp;&nbsp;&nbsp;&nbsp;
        <s:submit name="cancel" value="Cancel"></s:submit></div></td></tr>
          ...
          <div id="popupContact">
                 ...
                 <h1>Add Role</h1>
                 <p id="contactArea">
                       <s:form beanclass="com.inwise.action.RoleActionBean" id="saverole">
